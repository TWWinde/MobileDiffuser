...
struct StableDiffusionSample: ParsableCommand {
    ...
    @Option(help: "Override SD3 FlowMatch scheduler timestep shift (default: 3.0; distilled models often need 1.0)")
    var sd3TimestepShift: Float32? = nil
    ...
    mutating func run() throws {
        ...
        if #available(macOS 14.0, iOS 17.0, *) {
            if isXL {
                ...
                scaleFactor = 1.5305
                shiftFactor = 0.0609
                timestepShift = sd3TimestepShift ?? 3.0
                if !controlnet.isEmpty {
