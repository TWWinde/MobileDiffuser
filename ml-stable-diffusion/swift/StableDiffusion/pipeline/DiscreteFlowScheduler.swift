...
public final class DiscreteFlowScheduler: Scheduler {
    ...
    public init(
        ...
        // Direct uniform sampling in t-space [1.0, 0.0] with (stepCount + 1)
        // anchors, then apply the timestep shift exactly once. This matches the
        // canonical flow-match distilled-SD3 schedule, e.g. for 4 steps & shift=3:
        //   t     = [1.0, 0.75, 0.5, 0.25, 0.0]
        //   sigma = [1.0, 0.9,  0.75, 0.5,  0.0]
        let tDistribution = linspace(1.0, 0.0, stepCount + 1)
        self.sigmas = tDistribution.map { t -> Float in
            if timeStepShift == 1.0 { return t }
            return timeStepShift * t / (1.0 + (timeStepShift - 1.0) * t)
        }
        // MMDiT expects the timestep (in 0...trainSteps) at the *start* of each
        // step, hence dropLast() so we have exactly `stepCount` entries.
        self.timeSteps = tDistribution.dropLast().map { Int($0 * trainSteps) }
    }
    ...
    func timestepsFromSigmas() -> [Float] {
        // sigmas has (stepCount + 1) entries including a trailing 0; the
        // denoising loop runs stepCount times, so drop the final anchor.
        return sigmas.dropLast().map { $0 * trainSteps }
    }
    ...
    public func step(output: MLShapedArray<Float32>, timeStep t: Int, sample: MLShapedArray<Float32>) -> MLShapedArray<Float32> {
        ...
        let sigma = sigmas[stepIndex]
        // sigmas now has (stepCount + 1) entries including a trailing 0, so
        // `stepIndex + 1` is always in range.
        let prevSigma: Float = stepIndex + 1 < sigmas.count ? sigmas[stepIndex + 1] : 0
