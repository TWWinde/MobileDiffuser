...
public extension StableDiffusion3Pipeline {
    struct ResourceURLs {
        ...
        public let mmditConditioningURL: URL
        public let mmditAdaptiveLayerNormURL: URL
        public let mmditAdaptiveLayerNormStageURLs: [URL]
        public let mmditStageURLs: [URL]
        ...
        public init(resourcesAt baseURL: URL) {
            ...
            mmditConditioningURL = baseURL.appending(path: "MultiModalDiffusionTransformerConditioning.mlmodelc")
            mmditAdaptiveLayerNormURL = baseURL.appending(path: "MultiModalDiffusionTransformerAdaLN.mlmodelc")
            mmditAdaptiveLayerNormStageURLs = Self.numberedModelURLs(
                baseURL: baseURL,
                prefix: "MultiModalDiffusionTransformerAdaLNStage"
            )
            mmditStageURLs = Self.numberedModelURLs(
                baseURL: baseURL,
                prefix: "MultiModalDiffusionTransformerStage"
            )
        ...
        private static func numberedModelURLs(baseURL: URL, prefix: String) -> [URL] {
            ...
            return urls.compactMap { url -> (Int, URL)? in
                ...
                guard name.hasPrefix(prefix), name.hasSuffix(".mlmodelc") else {
                    return nil
                }
                let suffix = name
                    .dropFirst(prefix.count)
                    .dropLast(".mlmodelc".count)
    ...
    init(
        ...
        // Denoiser model. Prefer the split MMDiT layout when present; fall
        // back to the original monolithic model for compatibility.
        let mmdit: MultiModalDiffusionTransformer
        if FileManager.default.fileExists(atPath: urls.mmditConditioningURL.path),
           !urls.mmditStageURLs.isEmpty
        {
            ...
            mmdit = MultiModalDiffusionTransformer(modelAt: urls.mmditURL, configuration: config)
        }
