...
public struct StableDiffusion3Pipeline: StableDiffusionPipelineProtocol {
    ...
    public func prewarmResources() throws {
        print("[SD3] prewarm: textEncoder begin")
        try textEncoder.prewarmResources()
        print("[SD3] prewarm: textEncoder done")

        print("[SD3] prewarm: textEncoder2 begin")
        try textEncoder2.prewarmResources()
        print("[SD3] prewarm: textEncoder2 done")

        if textEncoderT5 != nil {
            print("[SD3] prewarm: textEncoderT5 begin")
            try textEncoderT5?.prewarmResources()
            print("[SD3] prewarm: textEncoderT5 done")
        }

        print("[SD3] prewarm: mmdit begin")
        try mmdit.prewarmResources()
        print("[SD3] prewarm: mmdit done")

        print("[SD3] prewarm: decoder begin")
        try decoder.prewarmResources()
        print("[SD3] prewarm: decoder done")
    ...
    public func generateImages(
        ...
        func logTiming(_ label: String, since start: CFAbsoluteTime) {
        ...
        print("[SD3] stage: before conditioning")
        let sd3Input: ModelInputs = try generateConditioning(using: config)
        print("[SD3] stage: after conditioning")
        logTiming("conditioning", since: conditioningStart)
        ...
        // Single-conditional fast path: when guidance is disabled (g < 1) AND
        // the MMDiT model was converted with batch=1, we skip the CFG batch
        // duplication, the unconditional branch, and the performGuidance blend.
        // This roughly halves MMDiT cost on device, which is the dominant term.
        let mmditBatch = mmdit.latentImageEmbeddingsShape.first ?? 2
        let useGuidance = config.guidanceScale >= 1.0 && mmditBatch >= 2
        let conditioningPrecomputeStart = CFAbsoluteTimeGetCurrent()
        let conditioningCache = try mmditModel.precomputeConditioning(
            timeSteps: timeSteps,
            batchSize: mmditBatch,
            pooledTextEmbeddings: mmditPooledStates
        )
        logTiming("conditioning precompute", since: conditioningPrecomputeStart)
        ...
        for (step, t) in timeSteps.enumerated() {
            ...
            let latentUnetInput: [MLShapedArray<Float32>] = useGuidance
                ? latents.map { MLShapedArray<Float32>(concatenating: [$0, $0], alongAxis: 0) }
                : latents
            ...
            var noise = try mmditModel.predictNoise(
                ...
                pooledTextEmbeddings: mmditPooledStates,
                precomputedConditioning: conditioningCache?[step]
            )
        ...
        logTiming("MMDiT loop", since: denoiseStart)

        // Decode the latent samples to images
        let decodeStart = CFAbsoluteTimeGetCurrent()
        let images = try decodeToImages(denoisedLatents, configuration: config)
        logTiming("VAE decode", since: decodeStart)
        logTiming("generate total", since: generateStart)
        return images
    }
    ...
    func encodePrompt(_ prompt: String) throws -> (MLShapedArray<Float32>, MLShapedArray<Float32>) {
        ...
        print("[SD3] stage: before textEncoder1")
        let (embeds1, pooledValue1) = try textEncoder.encode(prompt)
        print("[SD3] stage: after textEncoder1")
        if reduceMemory {
            textEncoder.unloadResources()
            print("[SD3] stage: unload textEncoder1")
        }

        print("[SD3] stage: before textEncoder2")
        let (embeds2, pooledValue2) = try textEncoder2.encode(prompt)
        print("[SD3] stage: after textEncoder2")
        if reduceMemory {
            textEncoder2.unloadResources()
            print("[SD3] stage: unload textEncoder2")
        }
    ...
    func generateConditioning(using config: Configuration) throws -> ModelInputs {
        ...
        // Skip negative-prompt encoding entirely when CFG is disabled OR the
        // MMDiT was converted with batch=1. Saves two TextEncoder passes per
        // generation.
        let mmditBatch = mmdit.latentImageEmbeddingsShape.first ?? 2
        let useGuidance = config.guidanceScale >= 1.0 && mmditBatch >= 2
        ...
        if !useGuidance {
            let pooledStates = MLShapedArray<Float32>(
                scalars: pooled.scalars,
                shape: [1, 2048, 1, 1]
            )
            return ModelInputs(hiddenStates: promptEmbedding, pooledStates: pooledStates)
        }
